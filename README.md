...
**Этап 1: Определение целей и параметров**
На этом этапе система собирает всю необходимую информацию о предстоящем мероприятии. Определяется тип события (ежедневное питание, семейный ужин, праздничное застолье или традиционная супра), количество гостей, их возрастные особенности и диетические ограничения. Система также учитывает сезонность и доступность продуктов в регионе пользователя.

**Этап 2: Подбор и балансировка блюд**
Система осуществляет интеллектуальный подбор блюд, обеспечивая баланс между различными категориями. Для закусок учитывается разнообразие вкусов и текстур, основные блюда подбираются с учетом белкового баланса, а десерты завершают гастрономический опыт. Алгоритм следит за культурной аутентичностью комбинаций и соблюдением традиционных грузинских принципов подачи блюд.

**Этап 3: Практическая организация**
На финальном этапе система формирует все необходимые практические материалы: детальный список покупок с расчетом количеств ингредиентов, пошаговый таймлайн приготовления с оптимизацией последовательности операций, и рекомендации по сервировке и подаче блюд. Пользователь получает готовый план, который позволяет без стресса организовать идеальное грузинское застолье.
...
javascript
{
  ...
  category: {type: String, enum: ['soup', 'main', 'appetizer', 'bread', 'sauce', 'dessert']},
  cuisine: {type: String, default: 'georgian'},
  difficulty: {type: String, enum: ['easy', 'medium', 'hard'], required: true},
  preparationTime: {type: Number, required: true}, // в минутах
